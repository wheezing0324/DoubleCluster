...
class CSAHFLServer(Server):
    def __init__(self, clients, device='cpu', environment=None):
        ...
        if self.environment is not None:
            self.edge_clients = self.environment.assign_edge_clients(self.num_edge_servers)
        else:
            ...
            chunks = np.array_split(cids, self.num_edge_servers)
            for i, chunk in enumerate(chunks):
                self.edge_clients[i] = [int(cid) for cid in chunk.tolist()]
    ...
    def get_checkpoint_state(self):
        ...
        state.update({
            ...
            "edge_clients": self.edge_clients,
    ...
    def run(self):
        ...
        for r in range(1, CONFIG['global_rounds'] + 1):
            ...
            for edge_id in range(self.num_edge_servers):
                ...
                if self.environment is not None:
                    selected_cids = self.environment.select_edge_clients(
                        r,
                        edge_id,
                        available_clients,
                        min(m_per_edge, len(available_clients)),
                    )
                else:
